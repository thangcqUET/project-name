...
static char test_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "Arduino";

#define TEST_MANUFACTURER_DATA_LEN  17

#define GATTS_DEMO_CHAR_VAL_LEN_MAX 0x40

#define PREPARE_BUF_MAX_SIZE 4096

#define IMAGE_MAX_SIZE 16*1024

static uint8_t char1_str[] = {0x11,0x22,0x33};

static uint16_t descr_value = 0x0;

uint8_t *image_data = NULL; // Pointer to hold image data for notifications
uint16_t image_data_len = 0; // Length of the image data
...
void example_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param){
    ...
    // ESP_LOGI(GATTS_TAG, "=== PREPARED WRITE DEBUG ===");
    // ESP_LOGI(GATTS_TAG, "Offset: %d", param->write.offset);
    // ESP_LOGI(GATTS_TAG, "Length: %d", param->write.len);
    // ESP_LOGI(GATTS_TAG, "Total: %d", param->write.offset + param->write.len);
    // ESP_LOGI(GATTS_TAG, "Buffer max: %d", PREPARE_BUF_MAX_SIZE);
    // ESP_LOGI(GATTS_TAG, "Current MTU: %d", local_mtu);
    // ESP_LOGI(GATTS_TAG, "Is prep: %s", param->write.is_prep ? "YES" : "NO");
    // ESP_LOGI(GATTS_TAG, "============================");
    if (param->write.need_rsp){
        if (param->write.is_prep) {
            if (param->write.offset > PREPARE_BUF_MAX_SIZE) {
                ESP_LOGE(GATTS_TAG, "Gatt_server prep write offset too large");
                status = ESP_GATT_INVALID_OFFSET;
            } else if ((param->write.offset + param->write.len) > PREPARE_BUF_MAX_SIZE) {
                ESP_LOGE(GATTS_TAG, "Gatt_server prep write length too large");
            ...
            if (gatt_rsp) {
                ...
                if (response_err != ESP_OK){
                    ...
                    // ESP_LOGI(GATTS_TAG, "Send response successfully\n");
                }
                free(gatt_rsp);
            } else {
        ...
        ESP_LOGI(GATTS_TAG, "Write without response, conn_id %d", param->write.conn_id);
        if (image_data == NULL) {
            image_data = (uint8_t *)malloc(IMAGE_MAX_SIZE * sizeof(uint8_t));
            if (image_data == NULL) {
                ESP_LOGE(GATTS_TAG, "Failed to allocate memory for image data");
                return;
            }
        }
        if (param->write.len > IMAGE_MAX_SIZE) {
            ESP_LOGE(GATTS_TAG, "Prepared data length exceeds maximum size");
            return;
        }
        if (param->write.len == 4 && param->write.value[0] == 0xFF && param->write.value[1] == 0xFF && param->write.value[2] == 0xFF && param->write.value[3] == 0xFF) {
            ...
            image_data_len += param->write.len; // Update the length of image data
            // log image_data length
            ESP_LOGI(GATTS_TAG, "Image data length: %d", image_data_len);
        }
    }
}
...
static char *convert_number_to_event_name(int event) {
        switch (event) {
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    // Add this at the beginning to log ALL events
    // ESP_LOGI(GATTS_TAG, "Profile A Event: %s", convert_number_to_event_name(event));
    switch (event) {
    ...
    case ESP_GATTS_EXEC_WRITE_EVT:
        ...
        //log length of prepared data
        ESP_LOGI(GATTS_TAG, "Prepared data length: %d", a_prepare_write_env.prepare_len);
    ...
    case ESP_GATTS_ADD_CHAR_DESCR_EVT:
        ...
        // Add this summary
        ESP_LOGI(GATTS_TAG, "=== SERVICE READY ===");
        ESP_LOGI(GATTS_TAG, "Service Handle: %d", gl_profile_tab[PROFILE_A_APP_ID].service_handle);
        ESP_LOGI(GATTS_TAG, "Char Handle: %d", gl_profile_tab[PROFILE_A_APP_ID].char_handle);
        ESP_LOGI(GATTS_TAG, "Descriptor Handle: %d", gl_profile_tab[PROFILE_A_APP_ID].descr_handle);
        ESP_LOGI(GATTS_TAG, "==================");
        break;
...
void ble_app_start(void)
{
    ...
    //log the status of the controller
    if (ret == ESP_BT_CONTROLLER_STATUS_IDLE) {
        ESP_LOGI(GATTS_TAG, "Bluetooth controller is idle");
    } else if (ret == ESP_BT_CONTROLLER_STATUS_INITED) {
        ESP_LOGI(GATTS_TAG, "Bluetooth controller is initialized");
    } else if (ret == ESP_BT_CONTROLLER_STATUS_ENABLED) {
        ESP_LOGI(GATTS_TAG, "Bluetooth controller is enabled");
    } else {
        ESP_LOGE(GATTS_TAG, "Bluetooth controller status unknown: %d", ret);
    }
